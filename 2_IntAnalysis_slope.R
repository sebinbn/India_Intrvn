...
# Creating data for different analysis ------------------------------------
...
# Creating data for initial regression
...
# Creating first diff data
diff_indx = !colnames(reg_dat) %in% c("Date", "Liq","Liq_3","Liq_5","D_Ann","D_Auc")
diff_dat = reg_dat #creating placeholder
diff_dat[-1,diff_indx] = apply(reg_dat[,diff_indx],2,diff)
# subsetting for TF estimation
tf_dat = diff_dat[Period[,"Pre"],]
# subsetting for Int analysis estimation
int_dat = diff_dat[Period[,"Int"],]
...
mod_s101 = arima(Merge_dat$s101[Period[,"Pre"]], order = c(0,1,1)) #1,1,0 > 0,1,1 in BIC. 1,1,1 has both insigniicant and of opposite signs.
mod_s101
BIC(mod_s101)
...
TF_101 = arima(tf_dat$s101, order = c(1,0,0),include.mean = F,
               xreg = tf_dat[c("WACR_12","Liq","Liq_5","EFFR_12","DGS10_5")])
...
Int_101 = arima(int_dat$s101, order = c(1,0,0),include.mean = F,
               xreg = int_dat[c("WACR_12","Liq","Liq_5","EFFR_12","DGS10_5","D_Ann")])
...
rm(Period,mod_s101,fitwhite, fitwhite1,op, ar101, reg_dat, diff_dat)
