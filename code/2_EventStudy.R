...
sd_1d = sd(Merge_dat$s101_1d[Merge_dat$Date >= as.Date("2018-04-01") &
                       Merge_dat$Date <= as.Date("2019-05-31")], na.rm = T)
sd_2d = sd(Merge_dat$s101_2d[Merge_dat$Date >= as.Date("2018-04-01") &
                             Merge_dat$Date <= as.Date("2019-05-31")], na.rm = T)
EvStudy_Ann$pval_1d = (1-pnorm(abs(EvStudy_Ann$D_1d)/sd_1d))*2  
EvStudy_Ann$pval_2d = (1-pnorm(abs(EvStudy_Ann$D_2d)/sd_2d))*2
n = length(Merge_dat$s101_1d[Merge_dat$Date >= as.Date("2018-04-01") &
                               Merge_dat$Date <= as.Date("2019-05-31")])
EvStudy_Ann$pval_1d = (1-pt(abs(EvStudy_Ann$D_1d)/sd_1d,n))*2  
EvStudy_Ann$pval_2d = (1-pnorm(abs(EvStudy_Ann$D_2d)/sd_2d))*2
pnorm(10)
pnorm(-0.083/sd_2d)
...
sd(Merge_dat$s101_1d[Merge_dat$Date >= as.Date("2018-04-01") &
                       Merge_dat$Date <= as.Date("2019-05-31")], na.rm = T)
